...
function getPage (page) {
	jQuery.getJSON('https://www.flickr.com/services/rest/?method=flickr.groups.pools.getPhotos&group_id='+gid+'&api_key='+key+'&per_page='+perpage+'&page='+page+'&format=json&nojsoncallback=1&auth_hash='+authhash,function (data) {
    var lastITem
    jQuery.each(data.photos.photo,function(i,item) {
        jQuery('#photo_'+item.id+' > div > span.photo_container').before('<div class="dateoverlay">'+formatDate(item.dateadded)+' P:' + page + '</div>');
    });
...
if (gid == undefined){
    gid = F.config.flickr.group.nsid;
    gid += "&user_id=26636301@N02";
}
...
jQuery("#yucs-top-list").append("<a href=\"" + document.location.href + "pool/26636301@N02/\">My Photos</a>")
